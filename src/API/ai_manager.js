...
export async function detectLanguageWithAI(text, supportedLanguages) {
    ...
    if (cleanText.length < 5) return 'English (US)';

    // Check cache
    const cacheKey = cleanText.slice(0, 50);
...
async function tryGemini(prompt) {
    for (const model of config.aiModels.gemini) {
        try {
...
async function tryGroq(prompt) {
    for (const model of config.aiModels.groq) {
        try {
...
async function tryOpenRouter(prompt) {
    for (const model of config.aiModels.openrouter) {
        try {
